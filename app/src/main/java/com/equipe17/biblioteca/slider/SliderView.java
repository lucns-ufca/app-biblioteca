package com.equipe17.biblioteca.slider;
...
public class SliderView extends FrameLayout {
    ...
    public boolean onBackPressed() {
        ...
        return ((FragmentView) getChildAt(lastIndex)).onBackPressed();
    }
    ...
    public int getCurrentIndex() {
        if (widthViewsPercentage == 0) {
            return (int) ((getX() * (-1)) / WIDTH_DISPLAY);
        } else if (spaceBetweenViews == 0) {
            return (int) (getX() / getWidthView());
        } else {
            int index = 0;
            int x = (int) getX() * (-1);
            for (int i = 0; i < getChildCount(); i++) {
